...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    public BaseGame(Context context, Handler handler){
        ...
        if(OfflineActivity.isMusicOn || MainActivity.isMusicNeeded){
            media_music.playBgm();
        }
    }
            ...
            for (AbstractEnemyAircraft enemyAircraft : enemyAircrafts) {
                ...
                if (enemyAircraft.crash(bullet)) {
                    ...
                    if (enemyAircraft.notValid()) {
                        ...
                        if(enemyAircraft instanceof BossEnemy && (OfflineActivity.isMusicOn || MainActivity.isMusicNeeded)){
                            media_music.playBgm();
                        }
                    }
                }
...
//        }
        ...
        if(heroAircraft.getHp() <= 0){
            ...
            if(MainActivity.isOnline){
                while(!MainActivity.isGameOverFlag()){
                    repaint();
                    try {
                        Thread.sleep(50);
                    } catch (InterruptedException e) {
    ...
    public void draw() {
        ...
        mSurfaceHolder.unlockCanvasAndPost(canvas); // 保证每次都将绘图的内容提交
    ...
    private void paintScoreAndLife() {
        ...
        if(MainActivity.isOnline){
            canvas.drawText("Enemy SCORE:" + MainActivity.getOpponentScore(), x+430, y, mPaint);
        }
    }

    @Override
    public void surfaceCreated(@NonNull SurfaceHolder mSurfaceHolder) {
        /*TODO*/
        mbLoop = true;
    ...
    @Override
    public void surfaceDestroyed(@NonNull SurfaceHolder mSurfaceHolder) {
        /*TODO*/
        mbLoop = false;
    }
